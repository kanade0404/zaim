...
func UpdateCategory(c echo.Context) error {
	...
	if err := usecases.NewUpdateCategoryUseCase(db, c.Logger()).UpdateCategories(ctx, zaimUserName); err != nil {
		c.Logger().Errorf("failed to update category: %v", err)
		return c.JSON(http.StatusInternalServerError, fmt.Errorf("failed to update category: %v", err))
	}
